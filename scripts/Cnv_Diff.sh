...
Rscript $path_Rscript'summary_diffCN_run.R' --fold_fun $path_Rscript --inputdir $path --ann_file $annotation_file --post ${post} --pre ${pre} --sample_name ${sample_name} --sex $sex --outf $path'summary_res/'
